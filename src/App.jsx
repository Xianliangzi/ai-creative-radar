...
const staticSignals = resources.map(normalizeResource)
...
function loadCustomResources() {
  ...
  try {
    const rawResources = window.localStorage.getItem(customResourceStorageKey)
...
function persistCustomResources(resourcesToSave) {
  window.localStorage.setItem(customResourceStorageKey, JSON.stringify(resourcesToSave))
}
...
function isLikelyUrl(input) {
  ...
  if (/^https?:\/\//i.test(trimmedInput)) return true
  return /^[\w-]+(\.[\w-]+)+([/?#].*)?$/i.test(trimmedInput)
}

function getCollectorInputType(input) {
  return isLikelyUrl(input) ? 'link' : 'text'
}

function normalizeCollectedResource(resource, inputType, inputValue) {
  const now = new Date().toISOString()
  const sourceStatus = inputType === 'link' ? '链接导入' : '文本导入'

  return {
    id: `local-resource-${Date.now()}`,
    title: resource.title || '未命名资料',
    type: resource.type || 'news',
    category: resource.category || 'AIGC 趋势',
    summary: resource.summary || '',
    source_name: resource.source_name || '',
    source_url: inputType === 'link' ? resource.source_url || inputValue : '',
    published_at: resource.published_at || '',
    collected_at: resource.collected_at || now,
    tools: Array.isArray(resource.tools) ? resource.tools : [],
    related_concepts: Array.isArray(resource.related_concepts) ? resource.related_concepts : [],
    target_users: Array.isArray(resource.target_users) ? resource.target_users : [],
    creator_value: resource.creator_value || '',
    project_ideas: Array.isArray(resource.project_ideas) ? resource.project_ideas : [],
    prompt_hint: resource.prompt_hint || '',
    workflow_hint: resource.workflow_hint || '',
    ...
    status: resource.status || '已入库',
    source_status: resource.source_status || sourceStatus,
    verification_status: resource.verification_status || '待核验',
    verification_note:
      resource.verification_note || '第一版仅做内容整理，真实性核验将在后续版本增强。',
    local_only: true,
  }
}
...
function findPlanMatches(query, sourceSignals = staticSignals) {
...
function App() {
  ...
  const [customResources, setCustomResources] = useState(loadCustomResources)
  ...
  const categories = useMemo(() => ['All', ...uniqueItems(signals.map((signal) => signal.category))], [signals])

  const planResults = useMemo(() => {
    return findPlanMatches(submittedPlanQuery, signals)
  }, [submittedPlanQuery, signals])
  ...
  }, [globalSearchTerm, savedPlans, signals])
  ...
  const runSmartSearch = async () => {
    ...
    const matchedResources = findPlanMatches(query, signals).slice(0, 12)
  ...
  const collectResource = async () => {
    const input = collectorInput.trim()
    const inputType = getCollectorInputType(input)

    if (!input) {
      setCollectorMessage('请先粘贴一个链接或一段内容。')
      return
    }

    setCollectorStatus(inputType === 'link' ? 'reading' : 'collecting')
    setCollectorMessage(inputType === 'link' ? '正在读取链接并整理资料...' : '正在用 AI 整理文本内容...')
    ...
    try {
      ...
      if (!response.ok || !data.success || !data.resource) {
        throw new Error(data.error || 'AI 暂时没有返回可用资料卡片')
      }

      const normalizedResource = normalizeCollectedResource(data.resource, inputType, input)
      setCollectorPreview(normalizedResource)
      setCollectorStatus('preview')
      setCollectorMessage('已整理为资料卡片，请预览后再保存。')
    } catch (error) {
  ...
  const saveCollectedResource = () => {
    if (!collectorPreview) return

    try {
      setCustomResources((currentResources) => {
        const nextResources = [collectorPreview, ...currentResources]
        persistCustomResources(nextResources)
        return nextResources
      })
      setCollectorStatus('saved')
      setCollectorMessage('已保存到本地资料库。它会进入资料列表、搜索、AI 灵感推荐和创意方案参考。')
      setCollectorInput('')
      setCollectorPreview(null)
    } catch (error) {
      setCollectorStatus('error')
      setCollectorMessage('本地保存失败，请检查浏览器存储权限后重试。')
    }
  }

  const deleteCustomResource = (resourceId) => {
    if (!window.confirm('确定要删除这条本地资料吗？')) return
  ...
  const clearCustomResources = () => {
    if (!window.confirm('确定要清空所有本地新增资料吗？')) return

    setCustomResources([])
    persistCustomResources([])
  }
