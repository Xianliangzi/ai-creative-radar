...
当前版本默认开放 AI 试用。后端保留可选 `AI_ACCESS_CODE` 机制，如果未来需要限制访问，可以在 Vercel 中配置该环境变量。
...
`AI_ACCESS_CODE` 是可选的访问保护口令：

- 如果设置了 `AI_ACCESS_CODE`，后端会要求 AI API 请求携带正确口令。
- 如果没有设置 `AI_ACCESS_CODE`，默认不启用口令保护，方便本地开发和调试。
- 当前前台页面不展示口令输入框，默认面向小范围测试开放使用。
- 该机制只是早期成本保护预留，不是正式账号系统，也不代表用户权限管理。
...
如果未来重新启用访问口令，真实口令也不能写死在前端代码中；校验应继续发生在 Serverless Function 中。
